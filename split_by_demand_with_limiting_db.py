...
def splitByDemand_wLimiting(stoich_matrix, rxnMat, prodMat, sumRxnVec, rho, pi, 
                            nutrientSet, Energy, Currency, Core, orgRxns, givenByps=[]):
    limitingDB = []
    # Initializing yield counters for E and B.
    runningE, runningB = 0.0, 0.0
    bypYields = {thisByp: 0.0 for thisByp in givenByps}
    ...
    while procRxnVec.any():
        ...
        for thisRxn in np.where(procRxnVec)[0]:
            ...
            for thisMet in mets[np.where(np.in1d(mets, np.array(Core + Energy + givenByps)))]:
                ratio = S[thisRxn, thisMet] / S[thisRxn, limRct]
                ...
                elif thisMet in givenByps:
                    bypYields[thisMet] += shareMatrix[thisRxn, limRct] * ratio
            ...
            # Updating the database of limiting reactants and reactions by depth.
            limitingDB.append((thisRxn, limRct))
        ...
        # Updating current network depth.
        currDepth += 1

    if isCoreProduced[Core].all() and not givenByps:
        return runningE, runningB, True, limitingDB
    elif givenByps and isCoreProduced[Core].all():
        return runningE, runningB, True, bypYields, limitingDB
    elif givenByps:
        return runningE, runningB, False, bypYields, limitingDB
    else:
        return -1.0, -1.0, False, limitingDB

def splitByDemand_wLimitingDepth(stoich_matrix, rxnMat, prodMat, sumRxnVec, rho, pi, 
                                 nutrientSet, Energy, Currency, Core, orgRxns, givenByps=[]):
    limitingDB = []
    # Initializing yield counters for E and B.
    runningE, runningB = 0.0, 0.0
    ...
    while procRxnVec.any():
        ...
        for thisRxn in np.where(procRxnVec)[0]:
            ...
            # Updating the database of limiting reactants and reactions by depth.
            limitingDB.append((thisRxn, limRct, currDepth))
...
def giveLimitingDB( orgRxns ):
    return splitByDemand_wLimiting( stoich_matrix, rxnMat, prodMat, 
                                    sumRxnVec, rho, pi, nutrientSet, 
                                    Energy, Currency, Core, orgRxns )[-1]

def giveLimitingDB_wDepth( orgRxns ):
    return splitByDemand_wLimitingDepth( stoich_matrix, rxnMat, prodMat, 
                                    sumRxnVec, rho, pi, nutrientSet, 
                                    Energy, Currency, Core, orgRxns )[-1]
